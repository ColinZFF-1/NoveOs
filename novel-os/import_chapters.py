...
CHAPTER_PATTERN = re.compile(r"第\s*(\d+)\s*章\s*[：:_]*(.+?)\s*_正文\.txt")
...
def import_chapters(db_path: Path, project_id: str, chapters_dir: Path, force: bool = False, dry_run: bool = False):
    ...
    files = sorted(chapters_dir.glob("第*章*_正文.txt"))
    if not files:
    ...
    for file_path in files:
        ...
        cursor.execute(
            """INSERT OR REPLACE INTO chapter_history
               ...
               VALUES (?, ?, ?, ?, ?, ?, datetime('now'))""",
            (project_id, info["chapter"], info["summary"], info["word_count"], "", info["title"]),
        )
        logger.info("导入 第 %d 章: %s (%d字)", info["chapter"], info["title"], info["word_count"])
        imported += 1
