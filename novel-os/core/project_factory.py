...
DEFAULT_BASE_PATH = Path(os.environ.get("NOVEL_BASE_PATH", "D:/noveos/books"))
...
class ProjectFactory:
    ...
    def create_from_outline(
        ...
        # 若 outline 未显式提供 chapters_target，按实际章节列表长度推导，避免默认 200 造成进度显示错误
        outline_chapters = len(outline.get("outline", []))
        chapters_target = int(outline.get("chapters_target", outline_chapters) or outline_chapters)
        words_per_chapter = int(outline.get("words_per_chapter", 2200) or 2200)
