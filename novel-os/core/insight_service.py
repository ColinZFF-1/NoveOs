...
5. 确保输出是合法的 JSON，可以直接被 Python json.loads 解析"""
    ...
    def _parse_json_robust(
        ...
        """强力解析 JSON，先标准解析，再修复，最后回退到 json5。"""
        try:
        ...
        try:
            ...
            logger.warning("JSON 修复后仍失败，尝试 json5: %s", exc2)
...
{{
  "summary": "整体结构说明",
  "volumes": [
    {{"index": 1, "title": "卷标题", "range": "1-30", "theme": "卷主题", "climax": "卷高潮"}}
  ],
  "outline": [
    {{
      "chapter": 1,
...
【输出要求】
1. 设计合理的卷数（建议每卷 10-20 章），每卷包含：卷号、标题、章节范围、主题、卷高潮
...
{{
  "summary": "整体结构说明",
  "volumes": [
    {{"index": 1, "title": "卷标题", "range": "1-15", "theme": "卷主题", "climax": "卷高潮"}}
  ],
  "characters": [
