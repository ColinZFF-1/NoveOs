    ...
    def execute(self, ctx: ChapterContext) -> StepResult:
        ...
        if ctx.book_config.llm.get("auditor_enabled", True):
            try:
                ...
                temp, max_tok = get_agent_llm_params(ctx.book_config, "auditor", 0.0, 2000)
                log_full_prompt("auditor", ctx.chapter_num, system, user)
                llm_report = ctx.llm.call_for_agent("auditor", system, user, temperature=temp, max_tokens=max_tok)
